...
class Vela_Widget_Popular_Posts extends WP_Widget {

	function __construct() {
		parent::__construct(
            'vela-popular-posts', 
            __('Vela: Popular Posts', 'Vela'), 
            array(
                'classname' => 'vela_widget_popular_posts', 
                'description' => "Popular Posts with Thumbnail."
            )
        );
	...
	function widget($args, $instance) {
		...
		$posts = new WP_Query( apply_filters( 'widget_posts_args', array(
                ...
                'meta_key' => 'wyde_post_views_count', 
...
<?php
	...
	function form( $instance ) {
        ...
        $defaults = array('title' => 'Popular Posts', 'number' => 5, 'show_date' => true);
...
add_action('widgets_init', 'vela_widget_popular_posts_load');

function vela_widget_popular_posts_load()
{
	register_widget('Vela_Widget_Popular_Posts');
}