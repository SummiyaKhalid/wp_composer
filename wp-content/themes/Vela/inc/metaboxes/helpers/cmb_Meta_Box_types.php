	...
	public function file_list() {
		...
		if ( $meta_value && is_array( $meta_value ) ) {
			...
			foreach ( $meta_value as $id ) {
                ...
                $thumb = wp_get_attachment_image_src( $id, $this->field->args( 'preview_size' ) );
                $fullurl = $thumb[0];
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					echo
					'<li class="img_status">',
						'<img width="100" height="100" src="'.$fullurl.'" alt="" />',
						'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button">'. __( 'Remove Image', 'cmb' ) .'</a></p>
						'. $id_input .'
					</li>';

				} else {
                    $ext = $this->get_file_ext($fullurl);
                    $title = basename($fullurl, ".".$ext);
					echo
					'<li>',
